...
export default async function ImpostoDeRendaPage() {
  const locale = await getLocale();
  const t = await getTranslations("calculators.imposto-de-renda");
  const tCommon = await getTranslations("common");
  const tCategories = await getTranslations("calculatorCategories");
  const tNav = await getTranslations("nav");
  const tSeo = await getTranslations("calculators.imposto-de-renda.seo");
