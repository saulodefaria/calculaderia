...
async function submitDefaultVehicleFinancing(page: Page) {
  ...
  await expect(page.getByText("R$ 894,00", { exact: false }).first()).toBeVisible();
  await expect(page.getByText("R$ 59.135,26", { exact: false }).first()).toBeVisible();
}
...
test.describe("financiamento-veiculo calculator", () => {
  ...
  test("submits the default Price estimate, shares explicit zero costs, restores, and preserves save callback", async ({
    ...
    expect(sharedParams.get("sv")).toBe("2026-06-25");
    expect(sharedParams.get("vv")).toBe("80000");
    expect(sharedParams.get("en")).toBe("20000");
    expect(sharedParams.get("cf")).toBe("0");
    expect(sharedParams.get("ca")).toBe("0");
    expect(sharedParams.get("tm")).toBe("1.49");
    expect(sharedParams.get("pm")).toBe("48");
    expect(sharedParams.get("mt")).toBe("price");
    ...
    await expect(visibleFieldById(restoredPage, "entrada")).toHaveValue("20.000,00");
    ...
    await expect(visibleFieldById(restoredPage, "taxaJurosMensal")).toHaveValue("1,49");
    await expect(visibleFieldById(restoredPage, "prazoMeses")).toHaveValue("48");
    await expect(restoredPage.locator("#metodo-price").filter({ visible: true }).first()).toHaveAttribute(
      "aria-pressed",
      "true"
    );
    await expect(restoredPage.locator("#compararMetodos")).toBeChecked();
    await expect(restoredPage.getByTestId("financiamento-veiculo-comparison")).toBeVisible();
    ...
    expect(callbackUrl).toContain("vv=80000");
    expect(callbackUrl).toContain("en=20000");
    expect(callbackUrl).toContain("cf=0");
    expect(callbackUrl).toContain("ca=0");
    expect(callbackUrl).toContain("tm=1.49");
    expect(callbackUrl).toContain("pm=48");
    expect(callbackUrl).toContain("mt=price");
    expect(callbackUrl).toContain("cmp=1");
  });
