...
export function ResultsSummary({ resultado, resultadoComAdicionais }: ResultsSummaryProps) {
  const t = useTranslations("calculators.financiamento.results");
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
        ...
        <div>
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            <div className="flex flex-col items-center justify-center rounded-lg border border-dashed p-3 text-center opacity-75">
              <span className="text-xs font-medium text-muted-foreground">{t("items.financedAmount")}</span>
              <span className="mt-1 text-base font-bold font-mono">
                {formatCurrency(resultadoComAdicionais.valorFinanciado)}
              </span>
            </div>
            <div className="flex flex-col items-center justify-center rounded-lg border border-dashed p-3 text-center opacity-75">
              <span className="text-xs font-medium text-muted-foreground">{t("items.totalInterest")}</span>
        ...
        {hasAluguelComAdicionais && (
          <div className="rounded-lg border border-purple-200 dark:border-purple-800 bg-purple-50/50 dark:bg-purple-950/30 p-4">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="flex items-center gap-2">
                <div>
                  <span className="text-xs text-muted-foreground block">{t("comparison.original.label")}</span>
                  <span className="text-sm font-semibold text-purple-600 dark:text-purple-400">
              ...
              <div className="flex items-center gap-2">
                <div>
                  <span className="text-xs text-muted-foreground block">{t("comparison.withExtras.label")}</span>
                  <span className="text-sm font-semibold text-purple-600 dark:text-purple-400">
            ...
            <p className="text-xs text-muted-foreground mt-3">{t("rent.comparisonNote")}</p>
          </div>
        )}
          ...
          <div className="rounded-xl border border-dashed bg-muted/40 dark:bg-muted/10 p-4 sm:p-5 space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-baseline sm:justify-between gap-2">
              ...
              <p className="text-xs text-muted-foreground">
                {hasAluguelComAdicionais ? t("irr.extrasExplanationWithRent") : t("irr.extrasExplanationWithoutRent")}
              </p>
            </div>
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="rounded-lg border bg-background p-3 space-y-2">
                ...
                <div className="space-y-1">
                  <div className="flex items-baseline justify-between gap-2">
                    <span className="text-xs text-muted-foreground">{t("irr.monthly")}</span>
                    <span
                  ...
                  <div className="flex items-baseline justify-between gap-2">
                    <span className="text-xs text-muted-foreground">{t("irr.annual")}</span>
                    <span
                      className={`text-lg font-bold font-mono ${
                        tirAnualOriginal! >= 0
                          ? "text-emerald-600 dark:text-emerald-400"
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <div className="flex flex-col items-center justify-center rounded-lg border-2 border-emerald-200 dark:border-emerald-800 bg-emerald-50/50 dark:bg-emerald-950/30 p-3 text-center">
              <span className="text-xs font-medium text-muted-foreground">
                {t("comparison.withExtras.newTotalInterest")}
              </span>
        ...
        <div className="rounded-xl bg-linear-to-r from-emerald-500 to-teal-500 p-4 text-white">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="flex flex-col">
              <span className="text-sm opacity-80">{t("savings.interest")}</span>
