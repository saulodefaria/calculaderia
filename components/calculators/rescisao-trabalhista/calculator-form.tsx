...
interface CalculatorFormProps {
  onCalculate: (inputs: InputsRescisaoTrabalhista) => void;
  initialValues?: InputsRescisaoTrabalhista | null;
}

const MOTIVOS: MotivoRescisao[] = ["semJustaCausa", "pedidoDemissao", "justaCausa", "acordo", "rescisaoIndireta"];
...
export function CalculatorForm({ onCalculate, initialValues }: CalculatorFormProps) {
  const t = useTranslations("calculators.rescisao-trabalhista.form");
  const defaults = useMemo(() => initialValues ?? getDefaultRescisaoTrabalhistaInputs(), [initialValues]);
  ...
  const [motivo, setMotivo] = useState<MotivoRescisao>(defaults.motivo);
  const [avisoPrevio, setAvisoPrevio] = useState<AvisoPrevio>(defaults.avisoPrevio);
  ...
  const [saldoFgts, setSaldoFgts] = useState(() => currencyInitial(defaults.saldoFgts));
  const [saldoFgtsIncluiVerbasRescisorias, setSaldoFgtsIncluiVerbasRescisorias] = useState(
    defaults.saldoFgtsIncluiVerbasRescisorias
  );
  ...
  const avisosPermitidos = getAvisosPermitidos(motivo);
  ...
  const handleMotivoChange = (value: string) => {
    const nextMotivo = value as MotivoRescisao;
    const nextAvisos = getAvisosPermitidos(nextMotivo);
    setMotivo(nextMotivo);
    setAvisoPrevio((current) => (nextAvisos.includes(current) ? current : getDefaultAvisoPrevio(nextMotivo)));
  };
  ...
  const buildInputs = (): InputsRescisaoTrabalhista => ({
    ...
    saldoFgts: saldoFgts.trim() ? parseCurrencyValue(saldoFgts) : undefined,
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    if (errors.length > 0 || inputs.salarioMensal <= 0) {
      setError(errors[0] ? t(`validation.${errors[0]}`) : t("validation.salarioMensal"));
      return;
    }
    ...
    <Card>
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          <section className="space-y-4" aria-labelledby="rescisao-contrato">
            <h2 id="rescisao-contrato" className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">
              {t("sections.contract")}
            </h2>
          ...
          <section className="space-y-4 border-t pt-5" aria-labelledby="rescisao-motivo-aviso">
            <h2
              id="rescisao-motivo-aviso"
              className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">
              {t("sections.reason")}
            </h2>
            <div className="grid gap-4 sm:grid-cols-3">
              <div className="space-y-2 sm:col-span-2">
                <Label htmlFor="motivo">{t("fields.motivo.label")}</Label>
                <Select value={motivo} onValueChange={handleMotivoChange}>
                  <SelectTrigger id="motivo" aria-label={t("fields.motivo.label")} className="w-full">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {MOTIVOS.map((option) => (
                      <SelectItem key={option} value={option}>
                        {t(`fields.motivo.options.${option}`)}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              <div className="space-y-2">
                <Label htmlFor="avisoPrevio">{t("fields.avisoPrevio.label")}</Label>
                <Select value={avisoPrevio} onValueChange={(value) => setAvisoPrevio(value as AvisoPrevio)}>
                  <SelectTrigger id="avisoPrevio" aria-label={t("fields.avisoPrevio.label")} className="w-full">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {avisosPermitidos.map((option) => (
                      <SelectItem key={option} value={option}>
                        {t(`fields.avisoPrevio.options.${option}`)}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
          ...
          <section className="space-y-4 border-t pt-5" aria-labelledby="rescisao-ferias-decimo">
            <h2
              id="rescisao-ferias-decimo"
          ...
          <section className="space-y-4 border-t pt-5" aria-labelledby="rescisao-fgts">
            <h2 id="rescisao-fgts" className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">
              {t("sections.fgts")}
            </h2>
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                <Label htmlFor="saldoFgts">{t("fields.saldoFgts.label")}</Label>
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-muted-foreground">R$</span>
                  <Input
                    id="saldoFgts"
                    inputMode="numeric"
                    value={saldoFgts}
                    onChange={(event) => handleCurrencyChange(event.target.value, setSaldoFgts)}
                    placeholder={t("fields.saldoFgts.placeholder")}
                    className="pl-10"
                  />
                </div>
              </div>
              <div className="flex items-center gap-3 rounded-md border p-3">
                <input
                  id="saldoFgtsIncluiVerbasRescisorias"
                  type="checkbox"
                  checked={saldoFgtsIncluiVerbasRescisorias}
                  onChange={(event) => setSaldoFgtsIncluiVerbasRescisorias(event.target.checked)}
                  className="h-4 w-4 rounded border-input"
                />
                <Label htmlFor="saldoFgtsIncluiVerbasRescisorias" className="text-sm leading-snug">
                  {t("fields.saldoFgtsIncluiVerbasRescisorias.label")}
                </Label>
              </div>
            </div>
          </section>

          <section className="space-y-4 border-t pt-5" aria-labelledby="rescisao-ajustes">
            <h2 id="rescisao-ajustes" className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">
