...
import type { FeriasBreakdownCategory, FeriasBreakdownRow } from "@/lib/calculators/ferias";

interface BreakdownTableProps {
  rows: FeriasBreakdownRow[];
}

const CATEGORY_ORDER: FeriasBreakdownCategory[] = ["recibo", "desconto", "fluxoSeparado"];

export function BreakdownTable({ rows }: BreakdownTableProps) {
  const t = useTranslations("calculators.ferias.breakdown");

  const renderDetail = (row: FeriasBreakdownRow) => {
    const details: string[] = [];
    if (row.dias !== undefined) details.push(t("details.days", { count: row.dias }));
    if (row.detalhe) details.push(t("details.avos", { value: row.detalhe }));
                  ...
                  <Fragment key={category}>
                    ...
                    {categoryRows.map((row) => (
                      <TableRow key={row.id}>
                        ...
                        <TableCell>{renderDetail(row) ?? t(`rows.${row.id}.description`)}</TableCell>
