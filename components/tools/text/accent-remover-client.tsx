...
import {
  ...
  type AccentRemovalMode,
  type AccentRemovalState,
  type AccentRemovalWarning,
} from "@/lib/tools/text";
...
const faqIds = ["privacy", "cedilla", "casing", "slug", "unchanged", "sharing", "compatibility"] as const;

function downloadText(value: string) {
  const url = URL.createObjectURL(new Blob([value], { type: "text/plain;charset=utf-8" }));
  const link = document.createElement("a");
  link.href = url;
  link.download = "removedor-acentos.txt";
  link.click();
  URL.revokeObjectURL(url);
}

export function AccentRemoverClient() {
  const searchParams = useSearchParams();
  const locale = useLocale();
  const t = useTranslations("tools.removedor-acentos.form");
  const tFaq = useTranslations("tools.removedor-acentos.faq");
  const hasReadContentFragment = useRef(false);
  const [state, setState] = useState<AccentRemovalState>(() =>
    readAccentRemovalStateFromParams(new URLSearchParams(searchParams.toString()))
  );
  ...
  const result = useMemo(
    () =>
      convertAccentRemoval(state.text, {
        mode: state.mode,
        locale,
      }),
    [locale, state.mode, state.text]
  );
  ...
  const metricRows = [
    ...
    ["removedMarks", result.removedMarks],
  ] as const;
  const statusDescription =
    result.status === "converted"
      ? t("result.convertedDescription", {
          mode: t(`modes.${result.modeApplied}`),
          removed: numberFormatter.format(result.removedMarks),
        })
      : result.status === "unchanged"
        ? t("result.unchangedDescription")
        : result.status === "tooLarge"
          ? t("result.tooLargeDescription", { limit: numberFormatter.format(ACCENT_REMOVAL_MAX_INPUT_LENGTH) })
  ...
  useEffect(() => {
    ...
    const contentFragment = readAccentRemovalContentFromFragment(window.location.hash);
  ...
  const updateState = (patch: Partial<AccentRemovalState>) => {
  ...
  return (
    <>
        ...
        <CardContent className="space-y-6">
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1.08fr)_minmax(320px,0.92fr)]">
            <div className="space-y-6">
              <section className="space-y-4 rounded-lg border p-4" aria-labelledby="accent-remover-input-title">
                <div>
                  <h2 id="accent-remover-input-title" className="font-semibold">
                    {t("input.title")}
                  </h2>
                ...
                <div className="space-y-2">
                  <Label htmlFor="accent-remover-input">{t("input.label")}</Label>
                  <textarea
                    id="accent-remover-input"
                    data-testid="accent-remover-input"
                ...
                <div className="flex flex-col gap-2 sm:flex-row">
                  <Button
                    ...
                    disabled={!state.text}
                    data-testid="accent-remover-copy-input">
                  ...
                  <Button
                    ...
                    disabled={!state.text}
                    data-testid="accent-remover-clear">
              ...
              <section className="space-y-4 rounded-lg border p-4" aria-labelledby="accent-remover-options-title">
                <div>
                  <h2 id="accent-remover-options-title" className="font-semibold">
                    {t("options.title")}
                  </h2>
                  <p className="mt-1 text-sm text-muted-foreground">{t("options.description")}</p>
                </div>

                <fieldset className="space-y-2" data-testid="accent-remover-mode-selector">
                  <legend className="text-sm font-medium">{t("modes.label")}</legend>
                  <div className="grid gap-2 sm:grid-cols-2">
                    {accentRemovalModes.map((mode) => (
                      <Button
                        key={mode}
                        type="button"
                        variant={state.mode === mode ? "default" : "outline"}
                        onClick={() => updateState({ mode: mode as AccentRemovalMode })}
                        aria-pressed={state.mode === mode}
                        data-testid={`accent-remover-mode-${mode}`}
                        className="min-h-11 w-full whitespace-normal text-left">
                        {t(`modes.${mode}`)}
                      </Button>
                    ))}
                  </div>
                  <p className="text-xs text-muted-foreground" data-testid="accent-remover-mode-description">
                    {t(`modeDescriptions.${state.mode}`)}
                  </p>
                </fieldset>
              </section>

              <section className="space-y-3 rounded-lg border p-4" aria-labelledby="accent-remover-share-title">
                <div>
                  <h2 id="accent-remover-share-title" className="font-semibold">
                    {t("share.title")}
                  </h2>
                ...
                <label className="flex items-start gap-2 text-sm">
                  <input
                    id="accent-remover-include-content"
                    data-testid="accent-remover-include-content"
                    type="checkbox"
                ...
                <p
                  data-testid="accent-remover-share-warning"
                  className={cn(
                    "text-sm",
                ...
                <div data-testid="accent-remover-share-button">
                  <ShareButton
                    className="w-full sm:w-auto"
                    getShareUrl={() => {
                      const shareUrl = buildAccentRemovalShareUrl(
            ...
            <aside className="space-y-5 rounded-lg border bg-muted/30 p-4" aria-labelledby="accent-remover-results">
              <div>
                <h2 id="accent-remover-results" className="font-semibold">
                  {t("result.title")}
                </h2>
              ...
              <div
                data-testid="accent-remover-status"
                aria-live="polite"
                className={cn(
              ...
              {result.warnings.length > 0 ? (
                <div className="flex flex-wrap gap-2" data-testid="accent-remover-warnings">
                  {result.warnings.map((warning: AccentRemovalWarning) => (
                    <span
                      key={warning}
                      data-testid={`accent-remover-warning-${warning}`}
              ...
              <div className="space-y-2">
                <Label htmlFor="accent-remover-output">{t("output.label")}</Label>
                <textarea
                  id="accent-remover-output"
                  data-testid="accent-remover-output"
                  className={textareaClassName}
                  value={result.output}
                  readOnly
              ...
              <div className="grid gap-2 sm:grid-cols-3">
                <Button
                  ...
                  data-testid="accent-remover-copy-output"
                  className="w-full">
                ...
                <Button
                  ...
                  disabled={!result.output}
                  data-testid="accent-remover-use-output"
                  className="w-full">
                ...
                <Button
                  ...
                  disabled={!result.output}
                  data-testid="accent-remover-download"
                  className="w-full">
              ...
              <div className="grid gap-3 sm:grid-cols-2" data-testid="accent-remover-metrics">
      ...
      <section className="mb-8 rounded-lg border p-6" aria-labelledby="accent-remover-faq">
        <h2 id="accent-remover-faq" className="text-2xl font-semibold tracking-tight">
          {tFaq("title")}
        </h2>
