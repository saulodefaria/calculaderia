...
- Validation results:
  ...
  - `DATABASE_URL=postgresql://postgres:postgres@localhost:5438/calculaderia?schema=public pnpm build` passed; Next listed `/[locale]/calculadoras/decimo-terceiro`.
  ...
  - `DATABASE_URL=postgresql://postgres:postgres@localhost:5438/calculaderia?schema=public pnpm exec next start --port 3100` served the built app; `curl -I http://localhost:3100/calculadoras/decimo-terceiro` returned 200 and a content smoke check found the title, form title, and `Tabelas INSS/IRRF 2026`.
...
- Tester validation results:
  ...
  - Tester started a separate built server with `DATABASE_URL=postgresql://postgres:postgres@localhost:5438/calculaderia?schema=public NEXT_TELEMETRY_DISABLED=1 AUTH_SECRET=manual-validation-secret AUTH_URL=http://localhost:3101 NEXTAUTH_URL=http://localhost:3101 pnpm exec next start --port 3101`.
