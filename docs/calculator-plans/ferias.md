...
- Validation results:
  ...
  - `PATH=/Users/saulodefaria/.nvm/versions/node/v22.21.1/bin:$PATH DATABASE_URL='postgresql://postgres:postgres@localhost:5433/calculaderia?schema=public' pnpm build`: passed. A build without `DATABASE_URL` stops at `prisma generate`, so the example Postgres URL from `.env.example` was supplied for compile validation.
