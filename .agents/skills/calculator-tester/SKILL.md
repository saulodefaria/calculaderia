...
2. Read the claimed DB item with `scripts/backlog/get_item.sql` when the orchestrator provides kind/slug, and treat the DB item as the source of truth for status/stage.
3. Inspect the route, field ids, result labels, share params, and save behavior before running broad tests.
4. Start `pnpm dev` for Browser/browser-use checks unless Playwright's configured web server is better for the requested test.
5. Use Browser/browser-use to verify:
...
6. Add or update Playwright tests only in `tests/e2e` when coverage is missing.
7. Run targeted e2e tests first, then broader e2e/lint/build checks when the plan requires them.
8. Update the plan with validation commands, browser coverage, failures, screenshots if useful, pass/fail status, and the DB item status/stage expected for orchestrator finalization.
